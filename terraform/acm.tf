...
resource "aws_acm_certificate" "redirect_virginia_cert" {
...
resource "aws_route53_record" "redirect_virginia_dns_validation_certificate" {
  for_each = {
    for dvo in aws_acm_certificate.redirect_virginia_cert.domain_validation_options : dvo.domain_name => {
      name   = dvo.resource_record_name
      record = dvo.resource_record_value
      type   = dvo.resource_record_type
    }
...
resource "aws_acm_certificate_validation" "redirect_virginia_validation" {
  # Most be virginia region
  provider                = aws.virginia
  certificate_arn         = aws_acm_certificate.redirect_virginia_cert.arn
  validation_record_fqdns = [for record in aws_route53_record.redirect_virginia_dns_validation_certificate : record.fqdn]
}
